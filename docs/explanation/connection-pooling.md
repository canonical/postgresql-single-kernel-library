...
If using PgBouncer is not enough to handle the connections load of your application, you can increase the amount of connections that PostgreSQL can open via the `experimental-max-connections` config parameter ([VM charm](https://charmhub.io/postgresql/configurations?channel=14/stable#experimental-max-connections)| [K8s charm](https://charmhub.io/postgresql/configurations?channel=14/stable#experimental-max-connections)).
