...
myst:
  html_meta:
    description: "Deploy Charmed PostgreSQL K8s across three availability zones on Google Kubernetes Engine using Juju zone constraints and pod anti-affinity rules."
---
...
This guide goes through an example deployment of PostgreSQL K8s on GKE (Google Kubernetes Engine) with Juju zone constraints/anti-affinity rules.
...
To survive a complete cloud outage, we recommend setting up {ref}`cluster-cluster-replication`.
