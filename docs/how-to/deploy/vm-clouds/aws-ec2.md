...
Follow the installation guides for [AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) - the Amazon Web Services CLI

To check they are all correctly installed, you can run the commands demonstrated below:
...
To access the database from outside of AWS open the AWS firewall using the [`juju expose`](https://juju.is/docs/juju/juju-expose) functionality:
...
Next, check and manually delete all unnecessary AWS EC2 instances, to show the list of all your EC2 instances run the following command (make sure the correct region is used!):
