...
You can check the [GCE instance availability](https://console.cloud.google.com/compute/instances) (make sure the right GCloud project is chosen!):
...
Next, check and manually delete all unnecessary GCloud resources, to show the list of all your GCE instances run the following command (make sure the correct region is used!):
...
Finally, remove GCloud JSON file user credentials to prevent any credential leakage:
