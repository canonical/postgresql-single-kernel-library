...
Charmed PostgreSQL is an open-source operator designed to deploy and operate PostgreSQL on machine clouds and Kubernetes. It packages the relational database management system [PostgreSQL](https://www.postgresql.org/) with the [Patroni](https://patroni.readthedocs.io/en/latest/) high-availability replication system into an operator for deployment with [Juju](https://juju.is/docs/juju).
