...
myst:
  html_meta:
    description: "Reference guide for Charmed PostgreSQL Juju application statuses, with expected operator actions for active, waiting, blocked, and error states."
---
...
| **blocked** | Please choose one endpoint to use. No need to relate all of them simultaneously! | {ref}`modern and legacy charm versions <charm-versions>` should not be used simultaneously. | Remove modern or legacy relation. Choose one to use at a time. |
