...
class RestoreManager(BaseManager):
    ...
    def _resolve_restore_target(
        ...
        """
        backups = self.backup_manager.get_backups(show_failed=False)
        timelines = self.backup_manager.get_timelines()
    ...
    def _fetch_backup_from_id(self, backup_id: str) -> str | None:
        ...
        return fetch_backup_from_id(
            backup_id, self.backup_manager.get_backups(show_failed=False, parse=False).keys()
        )
