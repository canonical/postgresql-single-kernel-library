...
def test_switchover(patroni):
    with (
        ...
        _post.assert_called_once_with(
            "https://endpoint:8008/switchover",
            json={"leader": "postgresql-k8s-0"},
...
def test_update_synchronous_node_count(peers_ips, patroni):
    with (
        ...
        patch("single_kernel_postgresql.managers.patroni.wait_fixed", return_value=wait_fixed(0)),
        ...
        patch(
            ...
            new_callable=PropertyMock,
            return_value=sentinel.synchronous_configuration,
        ),
    ):
        ...
        _patch.assert_called_once_with(
            "https://endpoint:8008/config",
            json=sentinel.synchronous_configuration,
            verify=patroni.verify,
            timeout=API_REQUEST_TIMEOUT,
            auth=patroni._patroni_auth,
        )
...
def test_member_started_true(peers_ips, patroni):
    with (
        ...
        patch(
            "single_kernel_postgresql.workload.vm.VMWorkload.is_patroni_running", return_value=True
        ),
        patch(
            "single_kernel_postgresql.workload.k8s.K8sWorkload.is_patroni_running",
            return_value=True,
        ),
    ):
...
def test_patroni_logs(substrate, patroni):
...
def test_last_postgresql_logs(substrate, patroni):
    if substrate == Substrates.K8S:
        pytest.skip("VM only")
        return
...
def test_get_patroni_restart_condition(substrate, patroni):
    if substrate == Substrates.K8S:
        pytest.skip("VM only")
        return
...
def test_update_patroni_restart_condition(substrate, patroni, new_restart_condition):
    if substrate == Substrates.K8S:
        pytest.skip("VM only")
        return
    ...
    with (
        ...
        patch("single_kernel_postgresql.managers.patroni.daemon_reload") as _daemon_reload,
    ):
        _open.return_value.__enter__.return_value.read.return_value = "Restart=always"
